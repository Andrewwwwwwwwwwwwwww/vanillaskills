 ...
 *
 * <p>Injected through the loot API rather than by overriding vanilla loot tables, so the vanilla files keep
 * whatever Mojang changes them to.
 *
 * <p>⚠ Edition-specific: Fabric uses {@code LootTableEvents.MODIFY}, NeoForge uses its own loot event.
 */
public final class ShardLoot {
    ...
    public static void register() {
        LootTableEvents.MODIFY.register((key, builder, source, registries) -> {
            if (CHESTS.contains(key) && GameplayConfig.SHARD_CHEST_WEIGHT > 0) {
                builder.withPool(pool(GameplayConfig.SHARD_CHEST_WEIGHT, GameplayConfig.SHARD_CHEST_EMPTY_WEIGHT));
            } else if (key.equals(BuiltInLootTables.PIGLIN_BARTERING) && GameplayConfig.SHARD_BARTER_WEIGHT > 0) {
                ...
                builder.withPool(pool(GameplayConfig.SHARD_BARTER_WEIGHT, GameplayConfig.SHARD_BARTER_EMPTY_WEIGHT));
            }
        });
    }
