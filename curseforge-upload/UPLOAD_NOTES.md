# CurseForge upload — VanillaSkills 2.1.7 (4 jars, no texture pack)

All four jars were **downloaded back from the published GitHub release** (`v2.1.7`) and byte-compared
...
> **No texture pack this time.** The pack did not change in 2.1.7 — the jars still point at the
...
**File:** `vanillaskills-2.1.7+mc26.2.jar` (this folder)
...
The CurseForge page jumps 2.0.1 → 2.1.7, so this covers all nine releases since:
...
- Existing worlds pick up both price changes automatically on update, and every player
...
VanillaSkills (the VSP Player/Server packs) from the 2.0.1 fileID to the 2.1.7 one and bump the pack
version.
