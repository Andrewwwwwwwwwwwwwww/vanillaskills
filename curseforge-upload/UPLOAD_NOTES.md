# CurseForge upload — VanillaSkills 2.1.15 (4 jars, no texture pack)

All four jars were **downloaded back from the published GitHub release** (`v2.1.15`) and byte-compared
...
> **No texture pack this time.** The pack did not change in 2.1.15 — the jars still point at the
...
**File:** `vanillaskills-2.1.15+mc26.2.jar` (this folder)
...
**File:** `vanillaskills-2.1.15+mc26.1.2.jar`
...
The CurseForge page jumps 2.0.1 → 2.1.15, so this covers all seventeen releases since:
...
- The config files explain themselves: gameplay.json and points.json carry an
  ...
  version.
- The Infusing Table pages instead of cutting off at 21 entries, lists the enchantments
  your held item can actually take first, and shows those as Enchanted Books.
...
VanillaSkills (the VSP Player/Server packs) from the 2.0.1 fileID to the 2.1.15 one and bump the pack
version.
